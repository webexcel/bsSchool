...
export class HomeworkComponent implements OnInit {
  ...
  selected(event: any) {
    this.sname = '';
    this.gethomework(event.CLASS_ID, event.NAME, event.ADNO);
  }
  ...
  gethomework(classid: any, name: any, adno: any) {
    this.homeworkdata = {};
    this.homeworkdates = [];
    ...
    this.authservice.post('getSaveHomeworkbyclass', storeData).subscribe(
      (result) => {
        ...
        if (this.resposeData.status) {
          this.homeworkdates = this.resposeData.data;
          // this.homeworkdates = [];
  ...
  checkimage(f: any) {
    if (f) {
      ...
      if (f != 'pdf' && f != 'mp3' && f != 'mp4' && f != 'xls' && f != 'xlsx' && f != 'wav' && f != 'docx' && f != 'doc') {
        return true;
      } else {
        return false;
      }
